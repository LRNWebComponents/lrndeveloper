...
items=("oops, didn't want to be here" "build a new component" "sync repos down from github" "pull all existing repos" "push to all existing repos" "rebuild all demo sites" "bower update all the things")
locations=('' 'lrn.sh' 'sync-down-github.sh' 'git-pull-all.sh' 'git-push-all.sh' 'update-all-demos.sh' 'bower-update-all.sh')
commands=('' 'sh' 'sh' 'sh' 'sh' 'sh')
